...
class ModelOutputError(RuntimeError):
    """Ответ получен, но формат не прошёл проверку; это результат эксперимента."""
...
class Run:
    def __init__(self, output, config, transport=None):
        ...
        self.config = {**config, 'model': self.model, 'base_url': self.base_url, 'thinking': 'disabled',
                       'data_mode': os.getenv('MACRO_DATA_MODE', 'snapshot'),
    ...
    def create(self, *, stage, messages, response_model, max_retries=3, validate=None,
        ...
        for attempt in range(1, max_retries + 2):
            ...
            event = {'stage': stage, 'attempt': attempt, 'cache_key': key, 'response_schema': response_model.__name__,
                     'request_sha256': digest(request), 'request': request, 'ticket': ticket}
            try:
            ...
            try:
                ...
                if attempt == max_retries + 1:
                    raise ModelOutputError(f'{stage}: не удалось получить валидный ответ за {attempt} попытки') from None
    ...
    def native(self, *, stage, messages, tools, temperature=0.0, max_tokens=1600):
        """Настоящий tool-calling, с отдельным кэшем для каждого повтора N."""
        ...
        path = self.output / 'cache' / (key + '.json')
        if path.exists():
            saved = read(path)
            return saved['value'], True, key
        ticket = self.reserve(request)
        started = time.perf_counter()
        event = {'stage': stage, 'cache_key': key, 'ticket': ticket, 'request': request,
                 'request_sha256': digest(request), 'attempt': 1}
        try:
            response = self.send(request)
        except Exception as exc:
            self.log({**event, 'status': 'api_error', 'error_type': type(exc).__name__,
                      'http_status': getattr(exc, 'status_code', None), 'seconds': time.perf_counter() - started})
            raise RuntimeError(f'API: {type(exc).__name__}, HTTP {getattr(exc, "status_code", "—")}. '
                               'Проверь ключ, баланс и сеть; прогресс сохранён.') from None
        usage = response.get('usage')
        self.settle(ticket, usage)
        choice = response['choices'][0]
        message = {k: v for k, v in choice['message'].items() if k in ('role', 'content', 'tool_calls') and v is not None}
        message.setdefault('role', 'assistant')
        event.update(response_id=response.get('id'), server_model=response.get('model'), usage=usage,
                     cost_upper_usd=usage_cost(usage), raw_response=message,
                     finish_reason=choice.get('finish_reason'), seconds=time.perf_counter() - started)
        if choice.get('finish_reason') not in ('stop', 'tool_calls'):
            self.log({**event, 'status': 'native_invalid'})
            raise ModelOutputError('Ответ tool-calling не завершён')
        self.log({**event, 'status': 'native_valid'})
        dump(path, {'stage': stage, 'response_id': response.get('id'), 'kind': 'native', 'value': message})
        return message, False, key
