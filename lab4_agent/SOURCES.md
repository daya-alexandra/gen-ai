...
- JSON-клиент с кэшем и аудитом адаптирован из лабораторной работы № 3; native tool calls добавлены по [документации DeepSeek](https://api-docs.deepseek.com/guides/tool_calls/). Планировщик/критик используют [JSON Output](https://api-docs.deepseek.com/guides/json_mode/).
...
Содержимое источников не изменяется, но доступ к FX скорректирован относительно стартера: выбирается запись не позже запроса, а не ближайшая по абсолютному расстоянию. Калькулятор использует ограниченный AST вместо eval/sympify. Схемы и реестр инструментов проверяются до вызова функции. Сохранённые результаты API и их содержательная проверка описаны в [API_REPORT.md](API_REPORT.md) и [API_REVIEW.json](API_REVIEW.json).
